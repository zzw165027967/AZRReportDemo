 ...
 */
function drawReport11Visualization()
{
	// Create and populate the data table.
	var data = new google.visualization.DataTable({
		cols : [{ id : '', label : 'Ontvanger (AW35/39)', type : 'string'
			}, { id : '', label : 'Verzender (AW35/39)', type : 'number'
    ...
    var filter1 = new google.visualization.ControlWrapper(addDefaultDropdownSelector({ 'containerId': 'divfilter1', 'options': { 'filterColumnLabel': 'Ontvanger (AW35/39)'} }));
    // Maak een filter aan voor 'Type'
    var filter2 = new google.visualization.ControlWrapper(addDefaultDropdownSelector({ 'containerId': 'divfilter2', 'options': { 'filterColumnLabel': 'Verzender (AW35/39)'} }));
    // Maak een filter aan voor 'Ontvanger'
    ...
    var graph1 = new google.visualization.ChartWrapper(addDefaultLineChart({ 'containerId': 'div1', 'options': { 'title': 'Ontvangen clienten per periode'} }));
    // Maak een grafiek aan voor suppliers
